...
const form = useForm({
    nombre: "",
    pais_id: null, // <-- El campo para la relación
});

const closeModal = () => emit("update:visible", false);

const submit = () => {
    form.post(route("admin.parametros.departamento.store"), {
        onSuccess: () => {
            emit("success", "Departamento creado.");
            closeModal();
            form.reset();
        },
    });
...
<template>
    ...
    >
        <form @submit.prevent="submit" class="p-fluid mt-4">
            ...
            <div class="field mb-4">
                <InputLabel for="pais_id" value="País" />
                <Dropdown
                    id="pais_id"
